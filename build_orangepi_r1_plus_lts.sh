...
IMAGE_NAME=eiftpi_orangepi_r1_plus_lts.img
function assert () {
    err=$1
    echo "FATAL: $err"
    exit 1
...
dd if=/dev/zero of=${IMAGE_NAME} bs=10M count=190 || assert
sgdisk -n 1:32768:+200M ${IMAGE_NAME}   || assert
...
git clone https://github.com/archlinuxarm/PKGBUILDs || assert
cd PKGBUILDs
git checkout 72ccc366bcefc10f53da6b313f28b63548bb9440
useradd builder
chmod -R 777 . || assert
...
cp /build/orangepi5/config_mainline_6.4.7 config
sed -i 's/_srcname=linux-6.2/_srcname=linux-6.4/g' PKGBUILD
sed -i 's/pkgver=6.2.10/pkgver=6.4.7/g' PKGBUILD
sed -i "s/'vboot-utils' //g" PKGBUILD
sed -i 's/ "${pkgbase}-chromebook"//g' PKGBUILD
sed -i 's/787862593d7bf354cf1a5c37e21fc147/bb65b2232cf596e7044c56a7c4205f51/g' PKGBUILD
sed -i 's/e0221ea0e6eeb147c29d2fd72e987ed5/046bacd50640ca59ac77c89c9fbf50f5/g' PKGBUILD
sed -i 's/f1379ae267804a7820963dd0527f11a8/c9d3c4becd2ea478a63aed285c00b521/g' PKGBUILD
sed -i 's_scripts/setlocalversion --save-scmversion_KERNELVERSION=6.4.7 scripts/setlocalversion --no-local_g' PKGBUILD
bash -c 'su -c aarch64-makepkg builder' || assert
cp linux*.pkg* / || assert
cd ../../../

echo "*** OrangePI: Compile TF-A ***"
git clone --depth 1 https://github.com/ARM-software/arm-trusted-firmware.git
cd arm-trusted-firmware
make realclean
make CROSS_COMPILE=aarch64-linux-gnu- PLAT=rk3328 -j$(nproc)
cd ..

echo "*** OrangePI: Compile u-boot ***"
...
cp u-boot-rockchip.bin /u-boot-orangepi.bin
cd ..
